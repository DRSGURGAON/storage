...
export interface SearchHit {
  ...
  status: string | null;
  /**
   * The record a hit is *read* on, when that is not the hit itself. A gate
   * pass has no screen of its own -- it is issued and shown on its
   * dispatch -- so a hit that could not say which dispatch would be a
   * result nobody can open. Null everywhere the id is the destination.
   */
...
export class SearchService {
  ...
  async search(actor: AuthenticatedUser, term: string, limit: number) {
    ...
    return withTenant(this.sql, actor.tenantId, async (tx) => {
      ...
      const rows = await tx<{ type: string; id: string; label: string; summary: string; status: string | null; parent_id: string | null; rank: number }[]>`
        with hits as (
          ...
          select 'product', p.id, p.sku, p.name || coalesce(' · HSN ' || p.hsn_code, ''),
                 case when p.is_active then 'active' else 'inactive' end, null,
          ...
          union all
          select 'vehicle', v.id, v.vehicle_number, coalesce(t.name, 'No transporter') || coalesce(' · ' || v.vehicle_type, ''), null, null,
          ...
          union all
          select 'grn', g.id, g.number, coalesce(c.legal_name, c.name) || ' · ' || w.code, g.status, null,
          ...
          union all
          select 'dispatch', d.id, d.number, coalesce(c.legal_name, c.name) || coalesce(' · ' || d.vehicle_number, ''), d.status, null,
          ...
          union all
          select 'gate_pass', gp.id, gp.number, 'Dispatch ' || d.number, gp.status, d.id,
          ...
          union all
          select 'pod', pod.id, pod.number, 'Dispatch ' || d.number || coalesce(' · ' || pod.receiver_name, ''), pod.status, null,
          ...
          select 'invoice', i.id, i.number,
                 coalesce(i.customer_snapshot->>'legal_name', i.customer_snapshot->>'name', '') || ' · ' || i.grand_total::text, i.status, null,
